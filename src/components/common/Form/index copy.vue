    ...
    <el-form ref="form" :model="formData" class="form">
      <el-row :gutter="20" v-for="item in form" :key="item.id">
        <el-col :span="span" v-for="formItem in item">
          <el-form-item :label="formItem.label" :prop="item.id" :rules="item.rules">
            <component :is="getComponentName(formItem.type)" :value="formData[formItem.id]"
              :disabled="formItem.isDisabled" :placeholder="formItem.placeholder" :options="formItem.options"
              :min="formItem.min" :max="formItem.max" :step="formItem.step" :format="formItem.format"
              :rows="formItem.rows" :maxlength="formItem.maxlength" :rules="formItem.rules"
              @blur="handleBlur(formItem.id)" @change="handleChange(formItem.id)"
              @upload-success="handleUploadSuccess(formItem.id)" @before-upload="beforeUpload" @remove="handleRemove">
            </component>
          </el-form-item>
        </el-col>
      </el-row>
